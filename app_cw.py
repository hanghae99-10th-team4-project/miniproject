...
client = MongoClient('mongodb+srv://test:sparta@cluster0.w6khe.mongodb.net/Cluster0?retryWrites=true&w=majority', tlsCAFile=ca)
db = client.dbsparta
...
@app.route('/api/member/setuser', methods=['GET'])
def set_user():
    token_receive = request.cookies.get('mytoken')
    payload = jwt.decode(token_receive, SECRET_KEY, algorithms=['HS256'])
    user_info = db.member.find_one({"user_id": payload['id']})
    return jsonify({'setUser': user_info['user_id']})
